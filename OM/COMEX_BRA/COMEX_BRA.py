...
class COMEX_BRA(ComexPipeline):
    """Pipeline de COMEX específico para Brasil."""
    ...
    def _download_file(self, file_info: Dict[str, str]) -> bytes:
        ...
        try:
            ...
            response = requests.get(file_info["url"], headers=headers, verify=False, stream=True, timeout=60)
...
def main():
    ...
    pipeline = COMEX_BRA(start_date=None)
    ...
    result_df.to_excel(f'subir_COMEX_BRA_{timestamp}.xlsx', index=False)
