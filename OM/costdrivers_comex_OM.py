...
class ComexPipeline(ABC):
    ...
    def calculate(self) -> pd.DataFrame:
        ...
        # Aplicar outlier_testing_iqr iterando por ID (evita problemas de tipo com groupby.apply no pandas 2.x)
        self.logger.info("Aplicando detecção e correção de outliers (IQR cumulativo)...")
        results = []
        for id_val, group in df_for_outlier.groupby('ID'):
            result = self.outlier_testing_iqr(group, date_column='Data', value_column='Valor')
            results.append(result)
        
        if results:
            df_suavizado = pd.concat(results, ignore_index=True)
        else:
            df_suavizado = pd.DataFrame(columns=['ID', 'Data', 'Valor', 'suavisado'])
        ...
        def guardrail_interpolate_time_series(
            ...
            for id_, group in df.groupby(id_col):
                ...
                for col in value_cols:
                    if col in group_full.columns:
                        # Garantir dtype numérico antes de interpolar (pd.NA gera object dtype)
                        group_full[col] = pd.to_numeric(group_full[col], errors='coerce')
...
def calculate_variation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Usar loop explícito para evitar perda de colunas com groupby.apply no pandas 2.x
    results = []
    for _, group in df.groupby(groupby_cols):
        results.append(calcular_variacao_percentual(group))
    
    if not results:
        return pd.DataFrame()
    
    df_resultado = pd.concat(results, ignore_index=True)
...
def calculate_new_values(df_resultado: pd.DataFrame, dataframe_costdriver: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"[DIAG] após concat df_ultimo: {len(df_ultimo)} + {antes_concat} = {len(df)} linhas")

    # Filtrar apenas IDs com mais de uma data
    antes_count = len(df)
    df = df[df.groupby('ID')['Data'].transform('count') > 1]
    logger.info(f"[DIAG] após filtro count>1: {antes_count} → {len(df)} linhas")
    # Remover linhas onde ambos Valor e Valor_Cif são NaN
    if 'Valor_Cif' in df.columns:
        antes_nan = len(df)
        df = df[~(df['Valor'].isna() & df['Valor_Cif'].isna())]
        logger.info(f"[DIAG] após filtro duplo-NaN: {antes_nan} → {len(df)} linhas")
...
def upload_comex(df_preenchido: pd.DataFrame, ExcluirHistorico: str = 'N', percentualOutlier: float = 60, logger=None) -> bool:
    """
    Realiza upload real dos dados COMEX para a API costdrivers.
    ...
    for ID, group in df_upload.groupby('ID'):
        ...
        # Remove colunas auxiliares que nao pertencem ao payload da API
        for col in ['suavisado', 'ImportExport']:
            if col in json_df.columns:
                json_df.drop(columns=[col], inplace=True)
    ...
    chunk_size = 500
    total = len(lista_requisicao)
    erros = []

    for ini in range(0, total, chunk_size):
        chunk = lista_requisicao[ini:ini + chunk_size]
        logger.info(f"Enviando lote {ini//chunk_size + 1} ({ini+1}–{min(ini+chunk_size, total)} de {total})...")
        resps = ApiAsync(True, chunk, headers=headers).run()
        for req, resp in zip(chunk, resps):
            if resp is None or 'Error' in str(resp):
                erros.append({'ID': req['ID'], 'resp': resp})

    if erros:
        logger.warning(f"Upload concluido com {len(erros)} erros:")
        for e in erros[:20]:
            logger.warning(f"  ID {e['ID']}: {e['resp']}")
    else:
        logger.info(f"=== UPLOAD CONCLUIDO — {total} IDs enviados com sucesso ===")
...
def upload_data(df: pd.DataFrame, ExcluirHistorico: str = 'N', percentualOutlier: float = 60) -> bool:
    """
    Realiza upload dos dados para a API costdrivers.
    ...
    """
    logger = logging.getLogger(__name__)
    logger.info("Iniciando upload real para a plataforma costdrivers...")
