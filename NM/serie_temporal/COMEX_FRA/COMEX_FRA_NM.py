...
class COMEX_FRA_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para França (FRA) usando Nova Metodologia.
    
    Processa dados da França a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'FR' ou REPORTER == 'FR'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados da França.
...
def main():
    ...
    pipeline = COMEX_FRA_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_FRA_NM_{timestamp}.xlsx', index=False)
