...
class COMEX_ITA_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Itália (ITA) usando Nova Metodologia.
    
    Processa dados da Itália a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'IT' ou REPORTER == 'IT'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados da Itália.
...
def main():
    ...
    pipeline = COMEX_ITA_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_ITA_NM_{timestamp}.xlsx', index=False)
