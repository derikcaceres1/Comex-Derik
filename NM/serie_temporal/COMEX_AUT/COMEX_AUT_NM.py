...
class COMEX_AUT_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Áustria (AUT) usando Nova Metodologia.
    
    Processa dados da Áustria a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'AT' ou REPORTER == 'AT'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados da Áustria.
...
def main():
    ...
    pipeline = COMEX_AUT_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_AUT_NM_{timestamp}.xlsx', index=False)
