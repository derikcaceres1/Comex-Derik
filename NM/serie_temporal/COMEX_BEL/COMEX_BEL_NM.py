...
class COMEX_BEL_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Bélgica (BEL) usando Nova Metodologia.
    
    Processa dados da Bélgica a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'BE' ou REPORTER == 'BE'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados da Bélgica.
...
def main():
    ...
    pipeline = COMEX_BEL_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_BEL_NM_{timestamp}.xlsx', index=False)
