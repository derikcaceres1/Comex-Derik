...
class COMEX_DEU_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Alemanha (DEU) usando Nova Metodologia.
    
    Processa dados da Alemanha a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'DE' ou REPORTER == 'DE'.
...
def main():
    ...
    pipeline = COMEX_DEU_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_DEU_NM_{timestamp}.xlsx', index=False)
