...
class COMEX_PRT_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Portugal (PRT) usando Nova Metodologia.
    
    Processa dados de Portugal a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'PT' ou REPORTER == 'PT'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados de Portugal.
...
def main():
    ...
    pipeline = COMEX_PRT_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_PRT_NM_{timestamp}.xlsx', index=False)
