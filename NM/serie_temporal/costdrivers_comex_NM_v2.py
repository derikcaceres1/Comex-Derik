"""
Framework COMEX com Nova Metodologia (NM) — versão 2.

Este módulo é a evolução de ``costdrivers_comex_NM.py``. Em relação ao v1
mantém **toda** a infraestrutura (Azure ADLS2, retry de upload, padronização
de tipos, otimização de memória, cache local, logs, validações) e adiciona
apenas o que foi acordado com o cliente:

1. **Filtros explícitos antes do cálculo** (fase ``filter_data``)
   — descarta NCMs sem mapeamento, IDs com histórico < 24 meses ou com gaps
   internos > 3 meses, e aplica top_n_percent. Gera ``silver.parquet`` e
   ``dropped.parquet`` com motivo de cada descarte.

2. **Histórico append-only** — ``update_historical`` usa o método
   ``_append_only`` (em vez de ``_update_historical_data`` do v1) para nunca
   sobrescrever meses já publicados.

3. **Upload opt-in** — ``NMConfig.allow_upload`` (padrão ``False``) +
   guarda em ``run()`` impedem qualquer envio acidental. O método ``upload()``
   em si é **idêntico** ao da v1 (fetch da API + filtro ``Data >= max_data``
   por ID + envio via ``option-9``).

Fluxo de execução:
    collect() -> update_historical() -> normalize_historical()
        -> filter_data() -> calculate() -> upload()

Tudo o que não toca essas mudanças permanece **idêntico** ao v1.
"""
...
# Imports do pacote v2 (módulos novos: NMConfig, motivos de descarte, filtros)
...
def reindex_series(series, date_column, value_column):
    ...
    # Reindex — .copy() garante objeto gravável no pandas 2.x (Copy-on-Write)
    series = series[~series.index.duplicated(keep='first')]
    series = series.reindex(full_index).copy()

    # replace 0 values with NaN (sem inplace para compatibilidade com CoW)
    series[value_column] = series[value_column].replace(0, np.nan)
...
class ComexPipelineNMv2(ABC):
    """
    Classe base abstrata para pipelines de COMEX com Nova Metodologia — v2.

    Mantém toda a infraestrutura do v1 (Azure ADLS2, retry de upload, padronização
    de tipos, cache local) e adiciona apenas:

    • ``filter_data()`` — fase explícita de filtros antes do cálculo
    • ``_append_only`` — histórico nunca sobrescreve meses publicados
    • ``NMConfig.allow_upload`` + guard em ``run()`` — upload opt-in

    O método ``upload()`` é **idêntico** ao da v1.
    """
    ...
    def __init__(self, iso_code: str,
                 ...
                 min_months_required: Optional[int] = None,
                 developing: bool = False,
                 iso_database: Optional[str] = None):
        """
        Inicializa o pipeline de COMEX com Nova Metodologia v2.

        Args:
            iso_code: Código ISO do país (ex: 'ARG', 'BRA', 'CHL')
            config: Instância de NMConfig com parâmetros do pipeline. Se None, usa
                    defaults (allow_upload=False, threshold=0.8, min_months=24,
                    max_gap=3, etc.).
            start_date: Data inicial para coleta (padrão: 150 dias atrás)
            ...
            threshold_percent: Sobrescreve config.top_percent_threshold se informado.
            min_months_required: Sobrescreve config.min_months_required se informado.
        ...
        # Prioridade: argumento explícito > config > default da config
        self.threshold_percent = (
            threshold_percent if threshold_percent is not None else self.config.top_percent_threshold
        )
        self.min_months_required = (
            min_months_required if min_months_required is not None else self.config.min_months_required
        )
        ...
        # Atributos novos do v2 (ausentes no v1) — apenas auditoria de filtros
        self.silver_df: Optional[pd.DataFrame] = None
        self.dropped_df: Optional[pd.DataFrame] = None
        self.gold_df = None

        self.logger.info(f"Pipeline NM v2 ({self.PIPELINE_VERSION}) inicializado para {self.iso_code}")
    ...
    def _update_historical_data(self, existing_historical: pd.DataFrame, 
        ...
        # Criar Data: usar coluna existente se disponível, senão derivar de CO_ANO/CO_MES
        if 'Data' not in existing_historical.columns:
            ...
            existing_historical['Data'] = pd.to_datetime(existing_historical['Data'])
        
        if 'Data' not in new_data.columns:
        ...
        # Determinar se Data é coluna do schema (não auxiliar)
        data_in_schema = 'Data' in list(existing_historical.columns[existing_historical.columns != 'Data_month'])
        cols_to_drop_aux = ['Data_month'] + (['Data'] if not data_in_schema else [])
        ...
        historical_old = historical_old.drop(columns=[c for c in cols_to_drop_aux if c in historical_old.columns])
        ...
        new_data_filtered = new_data_filtered.drop(columns=[c for c in cols_to_drop_aux if c in new_data_filtered.columns])
        ...
        if new_data_filtered.empty:
            ...
            existing_historical_clean = existing_historical_clean.drop(columns=[c for c in cols_to_drop_aux if c in existing_historical_clean.columns])
    ...
    # ------------------------------------------------------------------
    # APPEND-ONLY (substitui _update_historical_data no fluxo padrão do v2)
    # ------------------------------------------------------------------
    @staticmethod
    def _historical_keys(df: pd.DataFrame) -> List[str]:
        """Define a chave natural do histórico (Data/CO_ANO+CO_MES + ncm + pais_id + ImportExport)."""
        date_keys = (["Data"] if "Data" in df.columns
                     else (["CO_ANO", "CO_MES"] if "CO_ANO" in df.columns else []))
        return date_keys + [c for c in ("ncm", "pais_id", "ImportExport") if c in df.columns]

    def _append_only(self, existing: pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
        """Anexa apenas linhas com chave nova ao histórico existente.

        Diferente de :meth:`_update_historical_data`, este método **nunca** sobrescreve
        registros já gravados — atende a regra de negócio "dados de COMEX não devem
        ser revisados".

        Args:
            existing: histórico atual em disco
            new_df: dados convertidos para o schema do histórico

        Returns:
            DataFrame combinado, ordenado pela chave natural.
        """
        if existing.empty:
            self.logger.info("Histórico inexistente — utilizando apenas os dados novos.")
            return new_df.copy()
        if new_df.empty:
            self.logger.info("Nenhum dado novo — preservando histórico existente.")
            return existing.copy()

        keys = self._historical_keys(existing)
        if not keys:
            self.logger.warning("Não foi possível determinar a chave natural do histórico — preservando existente.")
            return existing.copy()
        ...
        if only_new.empty:
            self.logger.info("Nenhuma chave nova encontrada — append-only sem alterações.")
            return existing.copy()

        combined = pd.concat([existing, only_new], ignore_index=True).sort_values(keys)
        self.logger.info(
            f"Append-only: +{len(only_new)} linha(s) novas (total: {len(combined)})"
        )
    ...
    def _raw_to_historical_schema(self, df: pd.DataFrame, existing_historical: pd.DataFrame != None) -> pd.DataFrame:
        ...
        # Se as colunas históricas não estão presentes, aplicar normalização via data contract
        missing_cols = [c for c in historical_columns if c not in df_result.columns]
        if missing_cols:
            self.logger.info(f"Colunas históricas ausentes no raw_df: {missing_cols}. Aplicando normalização via data contract...")
            contract = self.load_data_contract()
            parts = []
            if 'ImportExport' in df_result.columns:
                for ie_val, ie_label in [(1, 'import'), (0, 'export')]:
                    subset = df_result[df_result['ImportExport'] == ie_val].copy()
                    if subset.empty:
                        continue
                    subset = self.normalize_columns(subset, contract, import_export=ie_val)
                    subset['ImportExport'] = ie_val
                    subset = self._country_specific_treatment(subset)
                    parts.append(subset)
            else:
                df_result = self.normalize_columns(df_result, contract, import_export=1)
                df_result = self._country_specific_treatment(df_result)
                parts.append(df_result)
            df_result = pd.concat(parts, ignore_index=True) if parts else pd.DataFrame()

        # Selecionar apenas colunas que existem; preencher as ausentes com NaN
        available = [c for c in historical_columns if c in df_result.columns]
        missing_final = [c for c in historical_columns if c not in df_result.columns]
        df_result = df_result[available]
    ...
    def update_historical(self, update_months: int = 3):
        ...
        # Histórico em modo APPEND-ONLY: meses já gravados nunca são sobrescritos.
        # Esta é a única diferença em relação ao v1 — a regra "dados de COMEX
        # não devem ser revisados" é honrada na fonte.
        self.logger.info("Atualizando histórico em modo append-only (nunca sobrescreve meses publicados)...")
        existing_size = len(existing_historical)
        updated_historical = self._append_only(existing_historical, raw_df_for_historical)
        updated_size = len(updated_historical)

        # Como _append_only nunca remove linhas, qualquer redução é sintoma de bug.
        if updated_size < existing_size:
            self.logger.error(
                "Bug em _append_only: histórico encolheu (de %s para %s). Preservando existente.",
                existing_size, updated_size,
            )
            self.historical_df = existing_historical.copy()
        ...
        if updated_size == existing_size:
            self.logger.info("Nenhuma chave nova nos dados coletados — histórico inalterado.")
            self.historical_df = existing_historical.copy()
            self.logger.info("=== ATUALIZAÇÃO DO HISTÓRICO CONCLUÍDA (sem novidades) ===\n")
            return
    ...
    # ============= FASE NOVA (v2): FILTER_DATA =============

    def filter_data(self) -> pd.DataFrame:
        """
        Aplica filtros explícitos antes do cálculo (fase nova do v2).

        Em sequência:
            F1. ``filter_ncm_mapped``  — só NCMs com IDIndicePrincipal mapeado
            F2. ``filter_min_history`` — só IDs com >= ``min_months_required`` meses
            F3. ``filter_max_gap``     — descarta IDs com gap interno > ``max_internal_gap_months``
            F4. ``top_n_percent``      — agrega países que somam até ``threshold_percent`` por peso

        Produz:
            • ``self.silver_df``  — dataframe pronto para o ``calculate``
            • ``self.dropped_df`` — IDs/NCMs descartados com motivo (auditoria)

        Salva ``silver_{date}.parquet`` e ``dropped_{date}.parquet`` em
        ``NM/dados/{ISO}/silver/``.

        Returns:
            DataFrame ``silver_df`` (entrada do ``calculate``).
        """
        ...
        if missing:
            self.logger.error("Colunas obrigatórias ausentes em historical_df: %s", missing)
            self.silver_df = pd.DataFrame()
            self.dropped_df = empty_dropped()
            return self.silver_df
        ...
        drops.append(dr)
        self.logger.info("F2 (min_history): %s pontos restantes, %s IDs descartados", len(df), len(dr))

        # F3 — gap interno máximo (regra do cliente: máx. 3 meses)
        ...
        non_empty = [d for d in drops if not d.empty]
        self.dropped_df = (pd.concat(non_empty, ignore_index=True)[DROPPED_COLUMNS]
                           if non_empty else empty_dropped())

        n_valid = self.silver_df[ID_COL].nunique() if not self.silver_df.empty else 0
        n_dropped = self.dropped_df[ID_COL].dropna().nunique() if not self.dropped_df.empty else 0
        self.logger.info(
            "Filtros concluídos: %s IDs válidos, %s IDs/NCMs descartados",
            n_valid, n_dropped,
        )
    ...
    @staticmethod
    def _apply_top_n_percent(df: pd.DataFrame,
                             threshold: float = 0.8) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """Aplica :func:`top_n_percent` (lógica do v1) e devolve também os IDs
        que tiveram pelo menos um país descartado, para auditoria.
        """
        if df.empty:
            return df.copy(), empty_dropped()

        countries_before = df.groupby([ID_COL, "Data"])["pais_id"].nunique().rename("n_before")
        aggregated = top_n_percent(df.copy(), threshold=threshold)
        if aggregated.empty:
            return aggregated, empty_dropped()

        countries_after = aggregated["pais_id_CUMUL"].astype(str).str.count(",") + 1
        kept = aggregated[[ID_COL, "Data"]].copy()
        kept["n_after"] = countries_after.values
        merged = kept.merge(countries_before.reset_index(), on=[ID_COL, "Data"], how="left")
        merged["n_dropped"] = (merged["n_before"] - merged["n_after"]).clip(lower=0)
        per_id = merged.groupby(ID_COL)["n_dropped"].sum()
        per_id = per_id[per_id > 0]
        if per_id.empty:
            return aggregated, empty_dropped()

        dropped = per_id.reset_index(name="_n").assign(
            ncm=pd.NA, ImportExport=pd.NA,
            reason=DropReason.NOT_IN_TOP_PERCENT.value,
            detail=lambda d: d["_n"].map(
                lambda n: f"{int(n)} país(es) descartado(s) por estarem fora do top {int(threshold * 100)}% por peso"
            ),
        )
        return aggregated, dropped[DROPPED_COLUMNS]
    ...
    def calculate(self) -> pd.DataFrame:
        ...
        for _attr in ("raw_import_df", "raw_export_df", "raw_df"):
            if getattr(self, _attr, None) is not None:
                setattr(self, _attr, None)
                self.logger.debug(f"{_attr} liberado da memória")
        gc.collect()
        self.logger.info("Limpeza de memória concluída")

        # No v2 a entrada do calculate é o silver_df produzido por filter_data().
        # Se filter_data não rodou, nós o disparamos para manter compatibilidade
        # com chamadas isoladas de calculate().
        if self.silver_df is None:
            self.logger.info("silver_df ausente — executando filter_data() automaticamente.")
            self.filter_data()
        ...
        series = self.silver_df.copy()
        self.logger.info("Entrada do cálculo: %s linhas, %s IDs únicos",
                         len(series), series[ID_COL].nunique())
        ...
        try:
            ...
            if not series_for_alpha.empty:
                _alpha_results = []
                for _, group in series_for_alpha.groupby('IDIndicePrincipal'):
                    _alpha_results.append(find_alpha(group.copy()))
                alpha = pd.concat(_alpha_results, ignore_index=True) if _alpha_results else pd.DataFrame(columns=['IDIndicePrincipal', 'Data', 'alpha'])
        ...
        # Garante float64 para evitar conflito de dtype com resultados STL (float64 vs float32)
        series_fob['Valor'] = series_fob['Valor'].astype(float)
        ...
        try:
            ...
            for _, group in series_fob.groupby('IDIndicePrincipal'):
                _results.append(preprocess_data(group.copy(), date_column='Data', value_column='Valor'))
            series_preprocessed = pd.concat(_results, ignore_index=True) if _results else pd.DataFrame()
        ...
        try:
            _results = []
            for _, group in series_preprocessed.groupby('ID'):
                _results.append(outlier_testing_stl(group.copy(), date_column='Data', value_column='Valor', id_column='ID'))
            series_clean = pd.concat(_results, ignore_index=True) if _results else pd.DataFrame()
            series_clean['ID'] = series_clean['ID'].astype(np.int32)
        ...
        # Garante float64 em colunas numéricas para evitar conflito de dtype nos passos seguintes
        for _col in ['Valor', 'Valor_final', 'Valor_clean']:
            if _col in series_clean.columns:
                series_clean[_col] = series_clean[_col].astype(float)

        # Final interpolation
        self.logger.info("Aplicando final_interpolation...")
        try:
            _results = []
            for _, group in series_clean.groupby('ID'):
                _results.append(final_interpolation(group.copy(), date_column='Data', value_column='Valor_final'))
            series_final = pd.concat(_results, ignore_index=True) if _results else pd.DataFrame()
            series_final['ID'] = series_final['ID'].astype(np.int32)
            # Garante float64 para evitar conflito com optimize_dtypes (float32 vs float64)
            for _col in series_final.select_dtypes(include='float32').columns:
                series_final[_col] = series_final[_col].astype(float)
        ...
        if not alpha.empty:
            ...
            final_cif['Valor_Cif'] = np.nan

        # Garante float64 para compatibilidade com interpolação no guardrail
        for _col in ['Valor', 'Valor_Cif']:
            if _col in final_cif.columns:
                final_cif[_col] = pd.to_numeric(final_cif[_col], errors='coerce')
        ...
        # Salvar resultado (nome do arquivo: gold_NM_v2_<data>.parquet)
        self.gold_df = final.copy()
        date_str = datetime.now().strftime('%Y-%m-%d')
        self._save_to_storage(self.gold_df, self.gold_path, f"gold_NM_v2_{date_str}.parquet")

        self.logger.info(f"=== FASE CALCULATE CONCLUÍDA: {len(self.gold_df)} registros ===\n")
        return self.gold_df
    ...
    def upload(self, ExcluirHistorico: str = 'N', percentualOutlier: float = 4) -> bool:
        ...
        # Guarda final do v2: nunca envia em modo developing (rede-safety).
        if self.developing:
            self.logger.warning("Modo developing ativo — upload abortado por segurança.")
            return False

        if self.gold_df is None or self.gold_df.empty:
            self.logger.warning("gold_df está vazio em memória. Tentando carregar gold mais recente do disco...")
            gold_dir = Path(self.gold_path)
            if gold_dir.exists():
                gold_files = sorted(gold_dir.glob("gold_NM_v2_*.parquet"), reverse=True)
                if not gold_files:
                    gold_files = sorted(gold_dir.glob("gold_NM_*.parquet"), reverse=True)
                if gold_files:
                    latest_gold = gold_files[0]
                    self.logger.info(f"Carregando gold do disco: {latest_gold}")
                    try:
                        self.gold_df = pd.read_parquet(latest_gold)
                        self.logger.info(f"✓ gold_df carregado: {len(self.gold_df)} registros")
                    except Exception as e:
                        self.logger.error(f"Erro ao carregar gold do disco: {e}")
                        return False
                else:
                    self.logger.error(f"Nenhum arquivo gold encontrado em: {gold_dir.absolute()}")
                    return False
            else:
                self.logger.error(f"Diretório gold não encontrado: {gold_dir.absolute()}")
                return False
    ...
    def _save_to_storage(self, df: pd.DataFrame, path: str, filename: str):
        ...
        # No v2 em modo developing sempre regravamos: o conteúdo pode ter mudado
        # mesmo com o número de linhas igual (ex.: ajustes em FOB/CIF de meses já existentes).
        if not self.developing and self._file_exists_in_storage(path, filename):
            try:
    ...
    def run(self, skip_phases: Optional[List[str]] = None) -> pd.DataFrame:
        """
        Pipeline completo do v2.

        Fluxo:
            1. collect()              — coleta dados brutos
            2. update_historical()    — append-only no histórico
            3. normalize_historical() — normaliza pelo data-contract.yaml
            4. filter_data()          — filtros explícitos (gera silver/dropped)
            5. calculate()            — STL, alpha, etc. (gera gold)
            6. upload()               — IDÊNTICO ao v1 (fetch API + filtro
                                        Data >= max_data por ID + envio).
                                        Só executa se ``config.allow_upload=True``
                                        E ``developing=False`` (double-guard).

        Args:
            skip_phases: lista de fases a pular (ex.: ``['upload']``).

        Returns:
            ``self.gold_df`` (ou DataFrame vazio em caso de falha).
        """
        skip_phases = skip_phases or []

        self.logger.info("=" * 60)
        self.logger.info(f"INICIANDO PIPELINE COMEX NM v2 - {self.iso_code}")
        self.logger.info(f"Versão {self.PIPELINE_VERSION} | developing={self.developing} | allow_upload={self.config.allow_upload}")
        self.logger.info("=" * 60)
        ...
        try:
            ...
            if 'filter_data' not in skip_phases:
                self.filter_data()
            else:
                self.logger.info("Fase 4 (filter_data) pulada")

            if 'calculate' not in skip_phases:
                self.calculate()
            else:
                self.logger.info("Fase 5 (calculate) pulada")

            # Double-guard: precisa estar fora de skip_phases, não-developing E allow_upload=True.
            # O upload em si tem o MESMO comportamento da v1 (fetch API + filtro Data >= max_data
            # + envio via option-9). A única diferença é a guarda opt-in: por padrão não envia.
            if 'upload' in skip_phases:
                self.logger.info("Fase 6 (upload) pulada via skip_phases")
            elif self.developing:
                self.logger.info("Fase 6 (upload) pulada — modo developing")
            elif not self.config.allow_upload:
                self.logger.info("Fase 6 (upload) pulada — config.allow_upload=False (opt-in)")
            else:
                self.upload()
            ...
            if hasattr(self, 'gold_df') and self.gold_df is not None and not self.gold_df.empty:
                return self.gold_df
            return pd.DataFrame()
