...
class COMEX_BGR_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Bulgária (BGR) usando Nova Metodologia.
    
    Processa dados da Bulgária a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'BG' ou REPORTER == 'BG'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados da Bulgária.
...
def main():
    ...
    pipeline = COMEX_BGR_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_BGR_NM_{timestamp}.xlsx', index=False)
