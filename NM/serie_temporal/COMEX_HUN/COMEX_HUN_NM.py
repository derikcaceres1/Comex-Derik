...
class COMEX_HUN_NM(ComexPipelineNM):
    """Pipeline de COMEX específico para Hungria (HUN) usando Nova Metodologia.
    
    Processa dados da Hungria a partir dos dados já coletados do EUR no ADLS,
    filtrando por pais_origem == 'HU' ou REPORTER == 'HU'.
    ...
    def _country_specific_treatment(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Tratamento específico para dados da Hungria.
...
def main():
    ...
    pipeline = COMEX_HUN_NM(start_date=None, developing=True)
    ...
    # result_df.to_excel(f'subir_COMEX_HUN_NM_{timestamp}.xlsx', index=False)
